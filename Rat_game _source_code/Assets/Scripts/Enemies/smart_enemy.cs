...
public class smart_enemy : MonoBehaviour
{
    ...
    void Update()
    {
        if(Vector2.Distance(transform.position, GetComponent<EnemyStatus>().player.transform.position) < 9f){follow = true;}
        else{follow = false;}
        if(follow){
            if(transform.localScale.x > 0 && transform.position.x > GetComponent<EnemyStatus>().player.transform.position.x){
        ...
        if(edgeHit == false && time > 0.5f && isGrounded() && !follow){
            StartCoroutine(flip());
            time = 0;
        }
        if(wallHit == true && time > 0.5f && isGrounded() && !follow){
            StartCoroutine(flip());
            time = 0;
        }
    }
